...
table.addEventListener('click', (e) => {
  if (e.target.classList.contains('get-teacher-btn-d')) {
    ...
    const url = `delete.html?id=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}&surname=${encodeURIComponent(surname)}&patronymic=${encodeURIComponent(patronymic)}&category=${Object.keys(categoryEnum).find(k => categoryEnum[k] === category)}`;
