...
func (d *DatabaseRepo) SelectAllGroups() (groups []model.Group, err error) {
	var tmp model.Group
	rows, err := d.db.Query(context.Background(), "SELECT id, specialization_id, name FROM group")
	if err != nil {
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		err = rows.Scan(&tmp.ID, &tmp.SpecializationID, &tmp.Name)
		if err != nil {
			return
		}
...
func (d *DatabaseRepo) SelectGroupByID(id int) (group model.Group, err error) {
	rows, err := d.db.Query(context.Background(), "SELECT id, specialization_id, name  FROM group WHERE id = $1", id)
	if err != nil {
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		err = rows.Scan(&group.ID, &group.SpecializationID, &group.Name)
		if err != nil {
			return
		}
	}
...
func (d *DatabaseRepo) SelectAllSpecializations() (specializations []model.Specialization, err error) {
	...
	rows, err := d.db.Query(context.Background(), "SELECT id FROM specialization")
	if err != nil {
		return
	}
	defer rows.Close()
...
func (d *DatabaseRepo) SelectSpecializationByID(id int) (specialization model.Specialization, err error) {
	rows, err := d.db.Query(context.Background(), "SELECT id FROM specialization WHERE id = $1", id)
	if err != nil {
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		err = rows.Scan(&specialization.ID)
		if err != nil {
			return
		}
	}
